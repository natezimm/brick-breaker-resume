...
describe('ui helpers', () => {
    ...
    test('startCountdown counts down then launches the ball', () => {
        ...
        jest.advanceTimersByTime(
            GAME_CONSTANTS.COUNTDOWN_INTERVAL * GAME_CONSTANTS.COUNTDOWN_START
        );
    ...
    test('createCountdownText renders countdown indicator', () => {
        document.body.innerHTML =
            '<div id="gameMessageOverlay" class="hidden"></div>';
    ...
    test('showGameOver and showWinMessage render messages', () => {
        ...
        expect(
            document.getElementById('gameMessageOverlay').innerHTML
        ).toContain('GAME OVER');
    ...
    test('setupUIButtons wires pause and high score controls', () => {
        ...
        expect(
            document
                .getElementById('highScoreModal')
                .classList.contains('active')
        ).toBe(true);
        expect(document.getElementById('highScoreValue').textContent).toBe(
            '42'
        );
    ...
    test('setupWindowResize updates UI elements on resize', () => {
        ...
        expect(gameState.scoreText.setPosition).toHaveBeenCalledWith(
            300 - 10,
            400 - 20
        );
